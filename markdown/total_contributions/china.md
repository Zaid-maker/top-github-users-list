...
The `public contributions` and `private contributions` by users in China on `2024/11/6 5:20 AM UTC`. This list contains users from `China` and cities `Beijing` `Shanghai` `Guangzhou` `Shenzhen` `Chongqing` `Chengdu` `Harbin` `Tianjin`.

There are `137 countries` and `666 cities` can be found [here](https://github.com/Zaid-maker/top-github-users-list).

There are `946 users`  in China. You need at least `731 followers` to be on this list.
