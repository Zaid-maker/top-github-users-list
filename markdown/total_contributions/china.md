...
The `public contributions` and `private contributions` by users in China on `2025/5/15 4:25 PM UTC`. This list contains users from `China` and cities `Beijing` `Shanghai` `Guangzhou` `Shenzhen` `Chongqing` `Chengdu` `Harbin` `Tianjin`.

There are `137 countries` and `668 cities` can be found [here](https://github.com/Zaid-maker/top-github-users-list).

There are `936 users`  in China. You need at least `770 followers` to be on this list.
