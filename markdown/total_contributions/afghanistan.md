...
The `public contributions` and `private contributions` by users in Afghanistan on `2024/9/18 5:18 AM UTC`. This list contains users from `Afghanistan` and cities `Kabul` `Kandahar` `Herat` `Kunduz` `Lashkargah` `Ghazni` `Khost` `Zaranj`.

There are `137 countries` and `666 cities` can be found [here](https://github.com/Zaid-maker/top-github-users-list).

There are `899 users`  in Afghanistan. You need at least `1 followers` to be on this list.
