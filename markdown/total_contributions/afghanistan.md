...
The `public contributions` and `private contributions` by users in Afghanistan on `2025/2/6 8:21 PM UTC`. This list contains users from `Afghanistan` and cities `Kabul` `Kandahar` `Herat` `Mazar-i-sharif` `Jalalabad` `Kunduz` `Ghazni` `Lashkar Gah` `Sheberghan` `Taloqan`.

There are `137 countries` and `668 cities` can be found [here](https://github.com/Zaid-maker/top-github-users-list).

There are `916 users`  in Afghanistan. You need at least `1 followers` to be on this list.
