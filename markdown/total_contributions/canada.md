...
The `public contributions` and `private contributions` by users in Canada on `2025/4/21 6:29 AM UTC`. This list contains users from `Canada` and cities `Toronto` `Montreal` `Vancouver` `Calgary` `Edmonton` `Quebec-city` `Winnipeg` `Saskatoon` `Regina`.

There are `137 countries` and `668 cities` can be found [here](https://github.com/Zaid-maker/top-github-users-list).

There are `930 users`  in Canada. You need at least `227 followers` to be on this list.
